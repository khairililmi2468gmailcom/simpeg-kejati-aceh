            ...
            <div class="w-full md:w-auto">
                <select name="per_page_mutasi" onchange="this.form.submit()"
                    ...
                    @foreach ([5, 10, 25, 50, 100, 250, 500, 1000, 2000, 5000, 10000] as $size)
                        <option value="{{ $size }}"
                            {{ request('per_page_mutasi', 5) == $size ? 'selected' : '' }}>
                            {{ $size }} / halaman
                        </option>
                    @endforeach
                </select>
    ...
    <table class="min-w-full bg-white border border-gray-300 rounded-b-lg">
        <thead class="bg-gray-100 text-[#02172C] font-poppins text-sm text-center">
            <tr class="uppercase">
                ...
                <th colspan="2" class="px-3 py-2 border border-gray-300 border-b-0">Jabatan</th>
                <th colspan="2" class="px-3 py-2 border border-gray-300 border-b-0">Unit Kerja</th>
                <th rowspan="2" class="px-2 py-2 border border-gray-300">Tanggal SK</th>
                <th colspan="2" class="px-0 py-2 border border-gray-300">TMT</th>
            ...
            @forelse ($mutasi as $index => $item)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-3 py-2 border border-gray-300">
                        <div class="font-medium text-black">{{ $item->pegawai->nama }}</div>
                        <div class="text-xs text-gray-500">NIP: {{ $item->pegawai->nip }}</div>
                        <div class="text-xs text-gray-500">NRP: {{ $item->pegawai->nrp }}</div>
                        <div class="text-xs text-gray-500">Karpeg: {{ $item->pegawai->karpeg }}</div>
                    </td>
                    ...
                    <td class="px-3 py-2 border border-gray-300 text-center">
                        {{ $item->pegawai->unitkerja->nama_kantor ?? '-' }}
                    </td>
                    <td class="px-3 py-2 border border-gray-300 text-center">
                        {{ \Carbon\Carbon::parse($item->tanggal_sk)->format('d-m-Y') }}
                    </td>
                    <td class="px-3 py-2 border border-gray-300">
                        {{ \Carbon\Carbon::parse($item->tmt_l)->format('d-m-Y') ?? '-' }}
                    </td>
                    <td class="px-3 py-2 border border-gray-300">
...
<div class="mt-2  flex justify-end">
    {{ $mutasi->appends(request()->all())->links() }}
</div>
@if ($mutasi->hasPages())
    ...
    <form method="GET" action="{{ route('admin.laporan.index') }}">
        ...
        <div class="flex flex-col md:flex-row md:space-x-4 mb-4">
            <div class="flex-1 relative">
                <input type="text" name="searchMutasi" placeholder="Masukkan NIP Pegawai"
                    value="{{ request('searchMutasi') }}" class="w-full border px-4 py-2 rounded-lg" />
            </div>
            <div>
                <button type="submit"
                ...
                <div class="w-full md:w-1/2 pr-5 relative">
                    <div class="relative">
                        <div class="selected-item w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm cursor-pointer flex justify-between items-center"
                            onclick="toggleDropdown('tahunmutasi')">
                            <span id="tahunmutasi-display">
                                -- Pilih Tahun Mutasi --
                            </span>
                        ...
                        <div id="tahunmutasi-dropdown"
                            ...
                            <input type="text" class="w-full px-3 py-2 border-b border-gray-200 focus:outline-none"
                                placeholder="Cari Tahun..." onkeyup="filterOptions('tahunmutasi', this.value)">
                            <div class="options" id="tahunmutasi-options">
                                @foreach ($tahunMutasi as $tahun)
                                    <div class="option-item px-3 py-2 hover:bg-gray-100 cursor-pointer transition-colors {{ request('tahun_mutasi') == $tahun ? 'bg-[#00A181] text-white hover:bg-[#009171]' : '' }}"
                                        data-value="{{ $tahun }}"
                                        onclick='selectItem("tahunmutasi", @json($tahun), @json($tahun))'>
                                        {{ $tahun }}
                                    </div>
                                @endforeach
                            </div>
                    ...
                    <input type="hidden" name="tahun_mutasi" id="tahunmutasi-input"
                        value="{{ request('tahun_mutasi') }}">
                </div>
            </div>
        ...
        <div class="mt-4 text-sm text-gray-600">
            ...
            <ul class="list-disc ml-6">
                <li>Masukkan Tahun Mutasi yang bersangkutan</li>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const tahun = document.getElementById('tahunmutasi-input').value;
        const items = document.querySelectorAll(`#tahunmutasi-options .option-item`);
        items.forEach(item => {
