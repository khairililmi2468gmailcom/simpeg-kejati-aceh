<div class="overflow-x-auto rounded-lg mb-2 ">
    <div class="flex items-center justify-between bg-[#00A181] px-6 py-4 rounded-t-lg">
        ...
        @php
            $queryParams = request()->only(['nip', 'no_sk', 'id_golongan']);
        @endphp
        <form method="GET" action="{{ route('admin.laporan.index') }}"
            ...
            <div class="w-full md:w-auto text-center md:text-right">
                <a href="{{ route('admin.laporan.kepangkatan.pdf', $queryParams) }}" target="_blank"
            ...
            @forelse ($kepangkatan as $index => $items)
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-4 py-3 border border-gray-300 text-center">
                        <a href="{{ route('admin.laporan.kepangkatan.single.pdf', $items->no_sk) }}" target="_blank"
...
@if ($kepangkatan->hasPages())
    <div class="mt-6 flex justify-end mb-4">
        <nav class="flex items-center space-x-1 text-sm">
            ...
            @endif
            @php
                $current = $kepangkatan->currentPage();
                $last = $kepangkatan->lastPage();
                $start = max($current - 2, 1);
                $end = min($current + 2, $last);
            @endphp
            {{-- Tampilkan "..." di awal jika halaman pertama tidak ditampilkan --}}
            @if ($start > 1)
                <a href="{{ $kepangkatan->url(1) }}"
