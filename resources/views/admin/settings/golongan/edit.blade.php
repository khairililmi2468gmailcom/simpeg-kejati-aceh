...
@section('content')
    ...
    <div class="max-w-3xl mx-auto mt-6 p-6 bg-white shadow-md rounded-xl">
        <form id="editForm" action="{{ route('admin.settings.golongan.update', $data->id_golongan) }}" method="POST"
            enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-4">
                <label for="id_golongan" class="block text-sm font-medium text-gray-700">Jabatan Fungsional</label>
                <input type="text" name="id_golongan" id="id_golongan"
                    value="{{ old('id_golongan', $data->id_golongan) }}" maxlength="25"
            ...
            <div class="mb-4">
                ...
                <input type="text" name="jabatan_fungsional" id="jabatan_fungsional"
                    value="{{ old('jabatan_fungsional', $data->jabatan_fungsional) }}" maxlength="25"
            ...
            <div class="mb-4">
                <label for="pangkat" class="block text-sm font-medium text-gray-700">Jabatan Fungsional</label>
                <input type="text" name="pangkat" id="pangkat"
                    value="{{ old('pangkat', $data->pangkat) }}" maxlength="25"
                    class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-[#00A181]">
                @error('pangkat')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
