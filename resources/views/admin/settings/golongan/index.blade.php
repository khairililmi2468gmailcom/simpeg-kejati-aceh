<h1 class="text-3xl font-bold text-[#00A181]">Daftar Golongan</h1>
<p class="text-gray-400 italic">Pages Golongan</p>
...
<div class="flex flex-wrap justify-between items-center mb-4 mt-4">
    ...
    <div class="w-full md:w-1/3 mb-4 md:mb-0 relative">
        <form action="{{ route('admin.settings.index') }}" method="GET">
            <input type="text" name="searchGolongan" value="{{ request('searchGolongan') }}"
    ...
    <div class="flex flex-wrap justify-end space-x-2">
        ...
        <a href="{{ route('admin.settings.golongan.create') }}"
            ...
            @forelse ($golongans as $item)
                <tr class="hover:bg-gray-50 transition-all duration-150">
                    <td class="px-5 py-4 text-center">
                        <input type="checkbox" class="checkbox-item" value="{{ $item->id_golongan }}">
                    </td>
                    <td class="px-5 py-4">{{ $item->jabatan_fungsional }}</td>
                    <td class="px-5 py-4">{{ $item->pangkat }}</td>
                    <td class="px-5 py-4 space-y-2">

                        <a href="{{ route('admin.settings.golongan.edit', $item->id_golongan) }}"
        ...
        bulkDeleteBtn.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.method = 'POST';
                    form.action = "{{ route('admin.settings.golongan.bulkDelete') }}";
