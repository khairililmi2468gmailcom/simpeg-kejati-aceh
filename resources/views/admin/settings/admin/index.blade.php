<h1 class="text-3xl font-bold text-[#00A181]">Daftar Admin</h1>
<p class="text-gray-400 italic">Pages Admin</p>
...
<div class="flex flex-wrap justify-between items-center mb-4 mt-4">
        ...
        <form action="{{ route('admin.settings.index') }}" method="GET">
            ...
            <select name="per_page_admin" onchange="this.form.submit()"
                ...
                @foreach ([5, 10, 25, 50, 100, 250, 500, 1000, 2000, 5000, 10000] as $size)
                    <option value="{{ $size }}" {{ request('per_page_admin', 5) == $size ? 'selected' : '' }}>
                        {{ $size }} / halaman
                    </option>
                @endforeach
            </select>
    ...
    <div class="flex flex-wrap justify-end space-x-2">
        ...
        <a href="{{ route('admin.settings.admin.create') }}"
            ...
            @forelse ($admins as $item)
                <tr class="hover:bg-gray-50 transition-all duration-150">
                    ...
                    <td class="px-5 py-4 space-y-2">
                        ...
                        <form action="{{ route('admin.settings.admin.destroy', $item->id) }}" method="POST"
        ...
        bulkDeleteBtn.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.method = 'POST';
                    form.action = "{{ route('admin.settings.admin.bulkDelete') }}";
