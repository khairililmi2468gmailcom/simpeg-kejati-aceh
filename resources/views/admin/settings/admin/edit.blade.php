...
@section('content')
    <div>
        <h1 class="text-3xl font-bold text-[#00A181]">Edit Admin</h1>
        <p class="text-gray-600">Edit informasi admin yang sudah ada.</p>
    </div>

    <div class="max-w-3xl mx-auto mt-6 p-6 bg-white shadow-md rounded-xl">
        <form id="editForm" action="{{ route('admin.settings.admin.update', $admin->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')

            <div class="mb-4">
                <label for="name" class="block text-sm font-medium text-gray-700">Nama Admin</label>
                <input type="text" name="name" id="name" value="{{ old('name', $admin->name) }}" maxlength="25" 
            ...
            <div class="mb-4">
                ...
                <input type="text" name="email" id="email" value="{{ old('email', $admin->email) }}" maxlength="25"
                    class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-[#00A181]"
...
@push('scripts')
    <script>
        ...
        document.getElementById('submitBtn').addEventListener('click', function() {
            Swal.fire({
                title: 'Edit Admin?',
                text: "Apakah Anda yakin ingin menyimpan perubahan pada admin ini?",
                icon: 'question',
