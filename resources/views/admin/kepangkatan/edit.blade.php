...
@section('content')
    <div>
        <h1 class="text-3xl font-bold text-[#00A181]">Edit Riwayat Kepangkatan</h1>
        <p class="text-gray-600">Perbarui informasi riwayat kepangkatan.</p>
    </div>

    <div class="max-w-3xl mx-auto mt-6 p-6 bg-white shadow-md rounded-xl">
        <form id="editForm" action="{{ route('admin.kepangkatan.update', $data->no_sk) }}" method="POST"
            ...
            <div class="mb-4">
                ...
                <div class="custom-select group relative">
                    <div class="selected-item flex items-center justify-between cursor-pointer w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm hover:border-gray-400"
                        ...
                        <span id="golongan-display">
                            {{ $golonganList->firstWhere('id_golongan', old('id_golongan', $data->id_golongan))?->nama_golongan ?? 'Pilih Golongan' }}
                        </span>
                        <svg class="w-5 h-5 text-gray-400 transform transition-transform" fill="none"
                            stroke="currentColor" viewBox="0 0 24 24" :class="{ 'rotate-180': dropdowns.golongan }">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                        </svg>
                    </div>
                    <div id="golongan-dropdown"
                        ...
                        <div class="options" id="golongan-options">
                            @foreach ($golonganList as $golongan)
                                <div class="option-item px-3 py-2 hover:bg-gray-100 cursor-pointer transition-colors {{ old('id_golongan', $data->id_golongan) == $golongan->id_golongan ? 'bg-[#00A181] text-white hover:bg-[#009171]' : '' }}"
                ...
                <input type="hidden" name="id_golongan" id="golongan-input"
                    value="{{ old('id_golongan', $data->id_golongan) }}">
                @error('id_golongan')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="no_pertimbangan" id="no_pertimbangan"
                    value="{{ old('no_pertimbangan', $data->no_pertimbangan) }}" maxlength="60"
                    class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-[#00A181]">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="pembuat_sk" id="pembuat_sk" value="{{ old('pembuat_sk', $data->pembuat_sk) }}"
                    maxlength="60"
                    class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-[#00A181]">
                @error('pembuat_sk')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="no_usulan" id="no_usulan" value="{{ old('no_usulan', $data->no_usulan) }}"
                    maxlength="50"
                    class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-[#00A181]">
                @error('no_usulan')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="jenis_usulan" id="jenis_usulan"
                    value="{{ old('jenis_usulan', $data->jenis_usulan) }}" maxlength="25"
                    class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-[#00A181]">
                @error('jenis_usulan')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            ...
            <div class="mb-4">
                ...
                <input type="text" name="pangkat_l" id="pangkat_l" value="{{ old('pangkat_l', $data->pangkat_l) }}"
                    maxlength="25"
                    class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-[#00A181]">
                @error('pangkat_l')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            {{-- Tanggal Mulai Tugas --}}
            <div class="mb-4">
                <label for="tmt_sk_pangkat" class="block text-sm font-medium text-gray-700">Terhitung Mulai Tanggal SK
                    Pangkat
                </label>
                <div class="relative max-w-sm">
                    <div class="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                    ...
                    <input name="tmt_sk_pangkat" id="tmt_sk_pangkat" type="date"
                        value="{{ old('tmt_sk_pangkat', $data->tmt_sk_pangkat) }}"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5">
                    @error('tmt_sk_pangkat')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
            </div>
