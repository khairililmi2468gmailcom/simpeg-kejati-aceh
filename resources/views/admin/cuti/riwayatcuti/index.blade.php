...
@section('content')
    <h1 class="text-3xl font-bold text-[#00A181]">Riwayat Diklat</h1>
    <p class="text-gray-600">Halaman Riwayat Diklat</p>
    ...
    @if ($menerimaCuti->hasPages())
        <div class="mt-6 flex justify-end">
            <nav class="flex items-center space-x-1 text-sm">
                ...
                @php
                    $current = $menerimaCuti->currentPage();
                    $last = $menerimaCuti->lastPage();
                    $start = max($current - 2, 1);
                    $end = min($current + 2, $last);
                @endphp
                {{-- Tampilkan "..." di awal jika halaman pertama tidak ditampilkan --}}
                @if ($start > 1)
                    <a href="{{ $menerimaCuti->url(1) }}"
