...
@section('content')
    <div>
        <h1 class="text-3xl font-bold text-[#00A181]">Edit Kabupaten</h1>
        <p class="text-gray-600">Perbarui Kabupaten</p>
    </div>
    ...
    <div class="max-w-xl mx-auto mt-6 p-6 bg-white shadow-md rounded-xl">
        <form action="{{ route('admin.kabupaten.update', $kabupaten->id) }}" method="POST">
            ...
            <div class="mb-4 relative">
                ...
                <div x-data="dropdown()" x-init="init('{{ old('id_provinsi', $kabupaten->id_provinsi) }}')" class="relative">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (submitBtn) {
            submitBtn.addEventListener('click', function (e) {
                ...
                const nama = document.getElementById('nama_kabupaten').value.trim();
                const jenis = document.querySelector('input[name="id_provinsi"]').value.trim();

                if (!nama || !jenis) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Lengkapi data',
                        text: 'Nama dan Provinsi wajib diisi.',
                        confirmButtonColor: '#00A181',
                    });
                    return;
                }
