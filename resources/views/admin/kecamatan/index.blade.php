...
@section('content')
    <h1 class="text-3xl font-bold text-[#00A181]">Kecamatan</h1>
    <p class="text-gray-600">Halaman Kecamatan</p>
    ...
    <div class="flex flex-wrap justify-between items-center mb-4 mt-4">
        ...
        <div class="w-full md:w-1/3 mb-4 md:mb-0 relative">
            <form action="{{ route('admin.kecamatan') }}" method="GET">
        ...
        <div class="flex flex-wrap justify-end space-x-2">
            ...
            <!-- Download Template -->
            <a href="{{ route('admin.kecamatan.download-template') }}"
        ...
        <table class="w-full text-base text-left text-gray-700 bg-white shadow-lg rounded-xl">
            <thead class="text-white bg-[#00A181]">
                <tr>
                    ...
                    <th class="px-5 py-4">ID Kecamatan</th>
                    <th class="px-5 py-4">Nama Kecamatan</th>
            ...
            <tbody class="divide-y divide-gray-200" id="provinsiTableBody">
                @forelse ($kecamatan as $item)
                    <tr class="hover:bg-gray-50 transition-all duration-150">
                        ...
                        <td class="px-5 py-4">{{ $item->nama_kecamatan }}</td>
                        <td class="px-5 py-4">{{ $item->kabupaten->id }}</td>
                        <td class="px-5 py-4">{{ $item->kabupaten->nama_kabupaten }}</td>
                        <td class="px-5 py-4 space-y-2">
                            ...
                            <form action="{{ route('admin.kecamatan.destroy', $item->id) }}" method="POST"
    ...
    @if ($kecamatan->hasPages())
        <div class="mt-6 flex justify-end">
            <nav class="flex items-center space-x-1 text-sm">
                ...
                {{-- Tombol Next --}}
                @if ($kecamatan->hasMorePages())
                    <a href="{{ $kecamatan->nextPageUrl() }}"
    ...
    <div id="importModal" class="fixed inset-0 bg-transparent bg-opacity-10 hidden flex items-center justify-center z-50">
        <div class="bg-white rounded-xl shadow-lg max-w-md w-full p-6 relative">
            ...
            <form id="importForm" action="{{ route('admin.kecamatan.import') }}" method="POST"
    ...
    <div id="referensiModal"
        ...
        <p class="text-sm text-gray-600 mb-4">
            <strong>Catatan:</strong> Gunakan ID Kabupaten sesuai referensi berikut untuk kolom <code>id_kabupaten</code> di
            file Excel.
        </p>

        <!-- Search -->
        <input type="text" id="searchInput" oninput="filterReferensiTable()" placeholder="Cari nama kabupaten..."
            ...
            <table id="referensiTableContent" class="min-w-full table-auto border">
                <thead>
                    <tr class="bg-gray-100 text-left text-xs text-gray-700 uppercase">
                        <th class="border px-3 py-2">ID Kabupaten</th>
                        <th class="border px-3 py-2">Nama Kabupaten</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($referensi['kabupaten'] as $kab)
                        <tr class="hover:bg-[#00A181] hover:text-white">
                            <td class="border px-3 py-1.5">{{ $kab->id }}</td>
                            <td class="border px-3 py-1.5">{{ $kab->nama }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        bulkDeleteBtn.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    form.method = 'POST';
                    form.action = "{{ route('admin.kecamatan.bulkDelete') }}";
